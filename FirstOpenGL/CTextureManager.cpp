...
bool CTextureManager::Create2DTextureFromASSIMPInfo(GLuint textureId, aiTexture* texture, const std::string& textureName) {
	...
	//TODO:: make a texture object and add it to the texture map
	CTextureUnitInfo info;
	info.texUnitNumber = m_textureNumber;
	info.texUnitNumOffsetBy_GL_TEXTURE0 = GL_TEXTURE0 + m_textureNumber;

	glBindTexture(GL_TEXTURE_2D, m_textureNumber);// Binding of texture name

	//NEW:: tell it how to store the texture
	...
	this->m_map_TexNameToTexUnit[textureName] = info;
