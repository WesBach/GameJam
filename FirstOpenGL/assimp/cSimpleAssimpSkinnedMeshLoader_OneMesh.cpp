...
glm::mat4 cSimpleAssimpSkinnedMesh::AIMatrixToGLMMatrix(const aiMatrix4x4 &mat){
...
float cSimpleAssimpSkinnedMesh::FindAnimationTotalTime(std::string animationName){
	...
	// Found it?
	if (itAnimation == this->mapAnimationNameTo_pScene.end()){	// Nope.
		return 0.0f;
	}
...
bool cSimpleAssimpSkinnedMesh::LoadMeshAnimation(const std::string &filename){
	//	std::map< std::string /*animationfile*/,
...
void cSimpleAssimpSkinnedMesh::BoneTransform(float TimeInSeconds,
	...
	std::vector<glm::mat4> &Globals,
	std::vector<glm::mat4> &Offsets,
	glm::vec3& handPosition, glm::vec3& handPositionR)
{
	...
	//time that use to move the mesh
	...
	this->ReadNodeHeirarchy(AnimationTime, animationName, this->pScene->mRootNode, Identity, handPosition, handPositionR);
	...
	glm::vec3& handPositionL, glm::vec3& handPositionR)
{
	...
	if (pNodeAnim){
		...
		//set the offset of the node here ?
		glm::mat4 TranslationM = glm::translate(glm::mat4(1.0f), pos);

		//TODO::
		//get both hand positions each time?
		if (NodeName == aiString("B_R_Hand")) {
			//set the position of the right hand
			handPositionR = TranslationM[3];
		}
		else if (NodeName == aiString("B_R_Hand")) {
			//set the position of the left hand
			handPositionL = TranslationM[3];
		}

		if (NodeName == aiString("B_Pelvis")) {
			TranslationM[3].x = 0.0f;
			TranslationM[3].z = 0.0f;
		}
	...
	for (unsigned int ChildIndex = 0; ChildIndex != pNode->mNumChildren; ChildIndex++){
		this->ReadNodeHeirarchy(AnimationTime, animationName,
			pNode->mChildren[ChildIndex], ObjectBoneTransformation, handPositionL, handPositionR);
	}
}
