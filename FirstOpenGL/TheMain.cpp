...
std::string lightFile = "assets/GameInfoFiles/LightInfo.txt";

cVAOMeshManager*	g_pVAOManager;
...
int main(int argc, char** argv) {
  ...
  /////////////////
	...
	if (!::g_pLightManager->LoadLightsAttributesFromFile(lightFile)) {
		error = "Lights not loaded properly! Line 260:main";
		printError(error);
	}
	//set the position for the shadow mapping (theres only one light)
  ...
  //do the texture loading
	...
	g_pTextureManager->LoadTexturesFromFile(std::string("assets/GameInfoFiles/TextureInfo.txt"));
	...
	if (!g_pFBO->init(width, 1060, error)) {
		std::cout << "FBO error: g_pFBO init :" << error << std::endl;
	}

	g_pFBOGeometry = new cFBO();
	//geometry stencil buffer
	if (!g_pFBOGeometry->init(width, 1060, error)) {
		std::cout << "FBO error: g_pFBOGeometry init: " << error << std::endl;
	}

	//set the initial values for the keys
	Keyboard::setKeys();

	//set the shader manager and bind the initial frame buffer
	::g_pShaderManager->useShaderProgram("mySexyShader");
	glBindFramebuffer(GL_FRAMEBUFFER, g_pFBOGeometry->ID);

	//get the light projection with a near and far plane
	...
	while (!glfwWindowShouldClose(g_pGLFWWindow)) {
		//makes sure the game pad is still connected and get all it's information
		...
		//set the camera positions to look at the player at all times
		...
		//move the light based on the player location (so the shadows follow the player)
		...
		//set the shader program and get the shader id
		::g_pShaderManager->useShaderProgram("mySexyShader");
		...
		//get the delta time for the keyboard functions
		...
		//determine the AI state behaviours
		...
		//check to see if projectiles are out of the range(to be removed)
		...
		//update all active tree emitters (so the particles will cast shadows)
		...
		// Set the screen width and height in the shader for sampling
		...
		//render the skybox with the entire scene on it
